...
//                 ecraft2learn.initialise_all();
      ...
      const stop_all_scripts = (and_reset_iframes) => {
          ...
          ecraft2learn.outstanding_callbacks.forEach(function (callback) {
              ...
              if (callback.ecraft2learn_top_block) {
                  callback.ecraft2learn_top_block.removeHighlight();
              }
          });
    ...
    const ancestor_of = (block) => {
        const block_bounds = block.fullBounds();
        let ancestor = block; // in case it has no ancestors
        const search_descendants = (block, possible_ancestor) => {
            if (possible_ancestor === block) {
                return true;
            } else {
                const ancestor_bounds = possible_ancestor.fullBounds();
                const overlapped = block_bounds.intersects(ancestor_bounds);
                if (overlapped) {
                    return true;
                }
//                 const is_ancestor = 
//                     possible_ancestor.children.some((child) => {
//                         // block might be a child or grandchild or ...
//                         if (search_descendants(block, child)) {
//                             return true;
//                         }
//                     });
//                 if (is_ancestor) {
//                     return true;
//                 } else if (possible_ancestor instanceof CommandBlockMorph && possible_ancestor.nextBlock()) {
//                     return search_descendants(block, possible_ancestor.nextBlock());
//                 }                      
            }
        };
        ecraft2learn.top_blocks_with_callbacks.some((top_block) => {
            if (search_descendants(ancestor, top_block)) {
                ancestor = top_block;
                return true;
            }
        });
    ...
    const record_callbacks = function () {
        const all_callbacks = Array.from(arguments);
        all_callbacks.forEach(function (callback) {
            if (callback && inside_snap() && callback instanceof Context) {
                callback.ecraft2learn_all_callbacks = all_callbacks; 
                if (ecraft2learn.current_process) {
                    callback.ecraft2learn_top_block = ancestor_of(ecraft2learn.current_process.topBlock);
                    // keep track of top blocks for block highlighting
                    const top_block_index = ecraft2learn.top_blocks_with_callbacks.indexOf(callback.ecraft2learn_top_block);
                    if (top_block_index < 0) {
                        ecraft2learn.top_blocks_with_callbacks.push(callback.ecraft2learn_top_block);
                    } 
                }
        ...
        if (ecraft2learn.current_process) {
            if (typeof ecraft2learn.current_process.topBlock.highlight_count === 'undefined' ||
                ecraft2learn.current_process.topBlock.highlight_count < 0) {
                ecraft2learn.current_process.topBlock.highlight_count = 0;
            }
//             if (//ecraft2learn.current_process.topBlock.highlight_count === 0 &&
//                 !ecraft2learn.current_process.topBlock.getHighlight()) {
                ecraft2learn.current_process.topBlock.addHighlight();
//             }
            // maintain count so when all nested callbacks finish highlight is removed 
            ecraft2learn.current_process.topBlock.highlight_count++;          
        }
    };
    let invoke_callback = function (callback) { // any number of additional arguments
        ...
        if (inside_snap() && callback instanceof Context) { // assume Snap! callback
            if (callback.stopped_by_user) { // should also handle paused by user 
                return;
            }
            ...
            if (callback.ecraft2learn_top_block) {
                if (callback.ecraft2learn_top_block.highlight_count > 0) {
                    callback.ecraft2learn_top_block.highlight_count--;
                }
//                 if (callback.ecraft2learn_top_block.highlight_count === 0) {
                    callback.ecraft2learn_top_block.removeHighlight();
//                 }
            }
            callback.ecraft2learn_all_callbacks.forEach((sibling) => {
                const location_of_this_callback = ecraft2learn.outstanding_callbacks.indexOf(sibling);
                if (location_of_this_callback >= 0) {
                    ecraft2learn.outstanding_callbacks.splice(location_of_this_callback, 1);
                }               
            });
...
//         };
    ...
    const optimize_hyperparameters = (model_name, number_of_experiments, epochs,
        ...
        if (trial_end_callback instanceof Context) {
            // this callback should have no influence on block highlighting 
            trial_end_callback.ecraft2learn_top_block = undefined; 
        }
        const time_stamp = Date.now();
        request_of_support_window('tensorflow.js',
                                  'Loaded',
...
// ["cmu-nk-hsmm", "NK Teluga female", "te"], // Teluga doesn't work with roman letters or digits
    ...
    return {
      ...
      start_speech_recognition: (final_spoken_callback, 
          ...
          if (interim_spoken_callback instanceof Context) {
              // this callback should have no influence on block highlighting 
              interim_spoken_callback.ecraft2learn_top_block = undefined; 
          }
          if (debugging) {
...
//               ecraft2learn.stop_speech_recognition();
    ...
    start_microsoft_speech_recognition: function (as_recognized_callback, final_spoken_callback, error_callback, provided_key) {
        ...
        if (as_recognized_callback instanceof Context) {
            // this callback should have no influence on block highlighting 
            as_recognized_callback.ecraft2learn_top_block = undefined; 
        }
...
//                                console.log(best_word, best_distance, words_considered);
  ...
  initialise_all: function () {
      ...
      ecraft2learn.top_blocks_with_callbacks = [];
      ecraft2learn.paused_callbacks = [];
