...
function drawSegment([ay, ax], [by, bx], color, scale, ctx) {
  ctx.beginPath();
  ctx.moveTo(canvasSize - ax * scale, ay * scale);
  ctx.lineTo(canvasSize - bx * scale, by * scale);
...
function drawPoints(ctx, points, radius, color) {
  ...
  for (let i = 0; i < data.length; i += 2) {
    ...
    if (pointX !== 0 && pointY !== 0) {
      ctx.beginPath();
      ctx.arc(canvasSize - pointX, pointY, radius, 0, 2 * Math.PI);
      ctx.fillStyle = color;
      ctx.fill();
    }
  }
...
function drawKeypoints(keypoints, minConfidence, ctx, scale = 1) {
  for (let i = 0; i < keypoints.length; i++) {
    ...
    ctx.beginPath();
    ctx.arc(canvasSize - x * scale, y * scale, 3, 0, 2 * Math.PI);
    ctx.fillStyle = color;
    ctx.fill();
  }
}
