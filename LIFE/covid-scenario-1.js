...
LIFE.context_sensitive_questions =
    [{scenario: 1,
      ...
      answer: "On a waiting bench in the hospital, at the waiting bay."},
     {scenario: 1,
      earliest_step: 9,
      latest_step: 9,
      questions: ["What is meant by ‘breathing adequacy’?",
                  "What does ‘breathing adequacy’ mean?"],
      answer: "If breathing seems right and with no abnormal findings to keep the patient alive."},
     {scenario: 1,
      earliest_step: 9,
      latest_step: 9,
      questions: ["What is meant by ‘assess the airway for secretions’?",
                  "How should one ‘assess the airway for secretions’?"],
      answer: "Assess the airway for secretions will usually be visually (check in the mouth and nose), " +
              "but can also be inferred by listening for noises from the upper airway (above the glottis). " + 
              "Such noises include gargling sounds."},
     {scenario: 1,
      earliest_step: 15,
      latest_step: 15,
      questions: ["What is meant by ‘A’ and ‘B’?",
                  "What is 'A' and 'B'?"],
      answer: "A is an abbreviation for Airway, B is an abbreviation for Breathing."},
     {scenario: 1,
      earliest_step: 20,
      latest_step: 20,
      questions: ["Why AVPU when the child is actively sitting on grandma’s lap, is alert and cooperates?",
                  "Why consider AVPU?"],
      answer: "AVPU is the simplest way to assess level of consciousness. " +
              "Although a child could have eyes open, this is not always similar to being alert (A) as alertness includes both eyes being open " + 
              "as well as the child being able to follow/respond to cues in the environment and being able to feed/drink. " +
              "The further you go away from A (I.e V-P-U), the lower the level of consciousness, and therefore the sicker the baby."},
     {scenario: 1,
      earliest_step: 22,
      latest_step: 23,
      questions: ["Why do we give antibiotics if Covid is a virus?",
                  "Since Covid is caused by a virus why do we give antibiotics?",
                  "Why are antibiotics given?"],
      answer: "Although viral infections are most common causes of acute respiratory illness/pneumonia in children, " +
              "we do not have a mechanism to determine if the ongoing pneumonia is actually caused by a virus " + 
              "or a community-acquired bacterial pneumonia or both. " + 
              "To reduce the risk of death from bacterial pneumonia (we give appropriate antibiotics for the disease severity)."},
     {scenario: 1,
      earliest_step: 14,
      latest_step: 29,
      questions: ["What can we use if we do not have an FFP3 mask? How often should we change our masks?",
                  "What other mask can we use?",
                  "Can we re-use our masks?"],
      answer: "N95 is the equivalent in our setting. " + 
              "N95 masks in our setting are now being reused (using a stepwise plan, where one healthcare worker has 3 masks, " + 
              "they use mask 1 on day 1, mask 2 on day 2, mask 3 on day 3 and restart the cycle. " + 
              "Surgical/medical masks are worn over the N95 to as well as a protective shield to protect N95 contamination. " + 
              "Surgical/medical masks should be changed when wet or after 4 hours, whichever comes first. " + 
              "They are non-reusable."},
     {scenario: 1,
      earliest_step: 14,
      latest_step: 29,
      questions: ["Why don’t we do chest x rays? Should we do an x ray if a baby is getting worse?",
                  "Should we do chest x-rays?"],
      answer: "Chest X-ray are being done in adult patients but not in children, " +
              "partly due to exposure to radiation, " + 
              "and due to the fact that more X-rays will not change the management plan. " +
              "However, Chest X-rays might have a role in helping arrive at alternative diagnoses such as TB " + 
              "(where for instance gene Xpert for sputum is not available). " + 
              "This is because all coughs are now eligible for checking if the child has TB."},
     {scenario: 1,
      earliest_step: 3,
      latest_step: 29,
      questions: ["If a mother or relative doesn’t have Covid, is it safe for her to be on the Covid isolation ward?",
                  "Is it safe for Covid-negative relatives to visit the Covid isolation ward?"],
      answer: "If the child is severely ill and meets the criteria for admission care, " + 
              "the baby will have the mother/relative room in as a way to promote family-centered care."},
     {scenario: 1,
      earliest_step: 15,
      latest_step: 29,
      ...
      answer: "This remains controversial in COVID-19. " + 
              ...
              "oral steroids or inhaled steroids are part of asthma management."},
     {scenario: 1,
      earliest_step: 7,
      latest_step: 8,
      questions: ["Why wouldn’t I give grandmother a gown AND a mask?",
                  "What is wrong with giving the grandmother a mask and gown?"],
      answer: "A gown is not appropriate PPE for the grandmother as she is not facing a similar exposure status comparable to healthcare workers."},
     {scenario: 1,
      earliest_step: 15,
      latest_step: 16,
      questions: ["Why the sniffing position?",
                  "Why prop up the head into the sniffing position?"],
      answer: "We consider neutral position for ages less than 1 year and sniffing position for ages one year and above."},
     {scenario: 1,
      earliest_step: 17,
      latest_step: 18,
      questions: ["Which bronchodilator should I give?",
                  "Which bronchodilator should be given?"],
      answer: "Inhaled bronchodilator (such as salbutamol)."},
     {scenario: 1,
      earliest_step: 17,
      latest_step: 18,
      questions: ["What bronchodilator dose should I give?",
                  "What is the recommended bronchodilator dose?"],
      answer: "The dose should be based on local guidelines appropriate for age and severity of asthma."},
     {scenario: 1,
      earliest_step: 17,
      latest_step: 18,
      questions: ["How should I administer the metered dose inhaler to a one year old since you would need a spacer device, as a one year old isn’t going to use an inhaler.",
                  "How can one administer the metered dose inhaler to a one year old?"],
      answer: "One would need a metered dose inhaler connected to an appropriate sized spacer for age."},
     {scenario: 1,
      earliest_step: 17,
      latest_step: 18,
      questions: ["How else might you deliver bronchodilator to a baby otherwise if there is no spacer device?",
                  "If no spacer devices are available, how can one deliver a bronchodilator to a baby?"],
      answers: "Improvised spacers are being used in some emergency rooms."},
     {scenario: 1,
      earliest_step: 18,
      latest_step: 21,
      questions: ["What are the signs of circulatory compromise?",
                  "What are the circulatory compromise signs?"],
      answer: "The signs include fast/slow pulse rate, cold extremities, weak/feeble peripheral pulses, low blood pressure " + 
              "(if can be taken), low level of mental status and reducing urine output."},
     {scenario: 1,
      earliest_step: 19,
      latest_step: 20,
      questions: ["How much fluid should be given in an IV fluid bolus?",
                  "How much fluid is recommended when giving an IV fluid bolus?"],
      answer: "This should be based on severity of circulatory compromise, guided by local paediatric guidelines."},    
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ['What are “other relevant tests” in this case?',
                  'What "other relevant tests" are there?'],
      answer: "Coughs will require checking for TB (sputum for gene Xpert). " + 
              "Diffuculty in breathing could be related to systemic diseases so a need to check, " + 
              "for instance Blood sugar for diabetic-Keto-Acidosis, Hb for severe anemia, and Malaria blood slide tests."},
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ["What if the baby is sleeping – they would be deemed ‘U’ on AVPU. How would you assess AVPU in a sleeping baby?",
                  "Should you try to wake the baby to access the 'U' in AVPU?"],
      answer: "Yes, all sleeping babies should be aroused. If they are unarousable, then they are classified as U."},
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ["Why use AVPU and not the GCS?",
                  "Why not use GCS instead of AVPU?"],
      answer: "AVPU is quick and easy to use in the outpatient emergency area. " + 
              "GCS is less familiar and more rigorous and therefore rarely used in outpatient emergency settings."},
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ["How would you assess the baby’s ability to feed?",
                  "How can I assess the baby's ability to feed?"],
      answer: "Ability to feed is assessed by offering the baby a cup to drink from or the mother’s breast to determine ability to breastfeed. " + 
              "The assessor will observe and made decision of able/not able to drink/breastfeed."},      
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ["What if the baby is breast fed? How would you assess feeding status in breast fed baby?",
                  "How can we assess the feeding status of a breastfed baby?"],
      answer: "If possible, the mother will be asked to offer the breast to the baby"},  
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ["Why would you check blood sugar?",
                  "Why check the blood sugar?"],
      answer: "Low blood sugar is high risk for death and should be identified and quickly corrected and prevented."},
     {scenario: 1,
      earliest_step: 20,
      latest_step: 21,
      questions: ["What are the indications for a chest X-ray in a baby with respiratory distress?",
                  "For a baby with respiratory distress what are the indications for a chest x-ray?"],
      answer: "Will be relevant for TB exclusion if we can't do gene Xpert, but this is not routine."},
     {scenario: 1,
      earliest_step: 22,
      latest_step: 23,
      questions: ["How do I calculate the IV maintenance fluids for a 1 year old?",
                  "What is the right amount of IV maintenance fluids for a 1 year old?"],
      answer: "Will be based on local guidelines (such as in Kenya, this is at 4ml/kg/24hours)."},
     {scenario: 1,
      earliest_step: 22,
      latest_step: 23,
      questions: ["What is the dose of paracetamol for a one year old?",
                  "What dose of paracetamol should be given to a one-year old?"],
      answer: "It depends upon local guidelines. For example, Kenya’s paediatric guidelines propose 10-15mg/kg every 6-8 hours."},
     {scenario: 1,
      earliest_step: 22,
      latest_step: 23,
      questions: ["Which antibiotics are broad spectrum and would be given here?",
                  "Which broad spectrum antibiotics should be given?"],
      answer: "Should be based on local guidelines and aimed at the prevailing bugs for community acquired pneumonia (not hospital acquired pneumonia). " + 
              "For Kenya, we propose high dose Amoxycilline (40-45mg/kg body weight in 2 divided doses, 12 hourly) " + 
              "Dispersible tablets for out-patient care-Non-severe pnumonia and crystalline penicillin/Gentamycin for inpatient care-severe pneumonia."},
];
