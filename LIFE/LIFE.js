// Machine learning to provide intelligent support for Oxford's LIFE game
// Written by Ken Kahn 
// License: New BSD

// following based upon sample code at 
...
const setup = () => {
    ...
    const obtain_embeddings = ((group_number, model) => {
        model.embed(group_of_questions[group_number]).then((embeddings) => {
            ...
            if (group_number+1 < group_of_questions.length) {
                obtain_embeddings(group_number+1, model);
            } else {
                console.log(group_of_questions_embeddings, group_of_questions_mean_embeddings);
                respond_to_questions(model);
            }
        });        
    });
    const respond_to_questions = ((model) => {
//         tf.metrics.cosineProximity()

    });
    use.load().then((model) => {
        obtain_embeddings(0, model);  
    });
};
